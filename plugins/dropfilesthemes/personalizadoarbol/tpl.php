...
?>
    ...
    <script type="text/x-handlebars-template" id="dropfiles-template-personalizadoarbol-box">
        {{#with file}}
        <div class="dropblock dropfiles-dropblock-content">
            ...
            <div class="filecontent">
                ...
                <div class="dropfiles-extra">
                    <?php if ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showdescription', 1) === 1) : ?>
                        {{#if description}}
                        ...
                        {{/if}}
                    <?php endif; ?>
                    <?php if ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showversion', 1) === 1) : ?>
                        {{#if versionNumber}}
                        ...
                        {{/if}}
                    <?php endif; ?>
                    <?php if ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showsize', 1) === 1) : ?>
                        {{#if size}}
                        <div><span><?php echo JText::_('COM_DROPFILES_DEFAULT_FRONT_SIZE'); ?> : </span> {{bytesToSize size}}
                        </div>
                        {{/if}}
                    <?php endif; ?>
                    <?php if ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showhits', 1) === 1) : ?>
                        {{#if hits}}
                        <div><span><?php echo JText::_('COM_DROPFILES_DEFAULT_FRONT_HITS'); ?> : </span> {{hits}}</div>
                        {{/if}}
                    <?php endif; ?>
                    <?php if ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showdateadd', 1) === 1) : ?>
                        {{#if created_time}}
                        ...
                        {{/if}}
                    <?php endif; ?>
                    <?php if ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showdatemodified', 0) === 1) : ?>
                        {{#if modified_time}}
...
<?php if ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showsubcategories', 1) === 1) : ?>
    <script type="text/x-handlebars-template" id="dropfiles-template-personalizadoarbol-categories">
        {{#if categories}}
        {{#each categories}}
...
<?php endif; ?>
    ...
    <script type="text/x-handlebars-template" id="dropfiles-template-personalizadoarbol-files">
        {{#if files}}
        {{#each files}}{{#if ext}}
    ...
    <script type="text/x-handlebars-template" id="dropfiles-template-personalizadoarbol-type">
        {{#if category}}
            {{#if category.type}}
...
<?php if ($this->category !== null) : ?>
    <?php if (!empty($this->files) || !empty($this->categories)) : ?>
        <div class="dropfiles-content dropfiles-content-multi dropfiles-files dropfiles-content-tree dropfiles-content-personalizadoarbol"
            ...
            if ($this->user_id) {
                ...
                <?php if ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showcategorytitle', 1) === 1) : ?>
                ...
                <?php if ($showdownloadcate === 1 && isset($this->category->linkdownload_cat) && !empty($this->files)) : ?>
                    <a data-catid="" class="personalizadoarbol-download-category download-all" href="<?php echo $this->category->linkdownload_cat; ?>"><span class="btn-title"><?php echo JText::_('COM_DROPFILES_DOWNLOAD_ALL'); ?></span><i class="zmdi zmdi-check-all"></i></a>
                <?php endif;?>
            </div>
            <?php $titlec = ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showtitle', 1) === 0) ? 'personalizadoarbol-hide-title' : '' ;?>
            <ul class="personalizadoarbol-list <?php echo  $titlec ;?>">
                <?php if (is_array($this->categories) && count($this->categories) &&
                          (int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showsubcategories', 1) === 1) : ?>
                            ...
                            <?php if ((int) $this->componentParams->get('custom_icon', 1) === 1 &&
                                ...
                                <li class="custom-icon <?php echo $file->ext; ?>">
                                    ...
                                    if (!$this->download_popup) {
                                        ...
                                        <?php if ((int) DropfilesBase::loadValue($this->params, 'personalizadoarbol_showtitle', 1) === 1) : ?>
                                            <?php echo $file->title; ?>
                                        <?php endif;?>
                                    </a>
                                </li>
